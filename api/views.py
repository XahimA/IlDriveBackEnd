...
from .serializers import CarDetailSerializer,UserdetailSerializer, TripDetailsSerializer, TripDetailsViewSerializer, UserIdSerializer, DriverDetailsViewSerializer, truckdetailSerializer, NewTripdetailSerializer
...
from rest_framework.response import Response
from rest_framework import status
from .serializers import DriverDetailsViewSerializer, DataDetailSerializer,OwnerDetailSerializer
# Create your views here.

def filter_func(object_name, id_val):
    filterd_obj = object_name.objects.filter(id = id_val).values()
    return filterd_obj
...
#     if request.method == 'POST':
#         serializer = DriverDetailsViewSerializer(data=request.data)
#         serializer.is_valid(raise_exception=True)
#         serializer.save()
#         return Response(serializer.data, status=status.HTTP_201_CREATED)


def filter_data(modelname,filterid, pk):
# Get data from database using pk 
    model_class = apps.get_model(app_label = 'api', model_name = modelname )
    column_name = f"{filterid}_id"
    filter_detail = model_class.objects.filter(**{column_name: pk}).values()   #filter data according to owner id
    if not filter_detail:
        filter_detail = get_object_or_404(model_class, **{column_name: pk}).values()
    else:
        filter_detail = model_class.objects.filter(**{column_name: pk}).values()
    return Response(filter_detail)


@api_view(['GET', 'POST', 'PUT'])
def driver_detail(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            try:
                driver_detail = DriverDetail.objects.get(pk=pk)
                serializer = DriverDetailsViewSerializer(driver_detail) 
                return Response(serializer.data)
            except DriverDetail.DoesNotExist:
                return Response(status=status.HTTP_404_NOT_FOUND)
        else:
            driver_details = DriverDetail.objects.all()
            serializer = DriverDetailsViewSerializer(driver_details, many=True)
            return Response(serializer.data)
    elif request.method == 'POST':
        serializer = DriverDetailsViewSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        try:
            driver_detail = DriverDetail.objects.get(pk=request.data['id'])
        except DriverDetail.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = DriverDetailsViewSerializer(driver_detail, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    else:
        return Response({"message": "Method not allowed. Please use 'GET', 'POST', or 'PUT'."},
                        status=status.HTTP_405_METHOD_NOT_ALLOWED)
    

@api_view(['GET', 'POST', 'PUT'])
def truck_detail(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            try:
                truck_details = truck.objects.get(pk=pk)
                serializer = truckdetailSerializer(truck_details) 
                return Response(serializer.data)
            except truck.DoesNotExist:
                return Response(status=status.HTTP_404_NOT_FOUND)
        else:
            truck_details = truck.objects.all()
            serializer = truckdetailSerializer(truck_details, many=True)
            return Response(serializer.data)
    elif request.method == 'POST':
        serializer = truckdetailSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    elif request.method == 'PUT':
        try:
            truck_details = truck.objects.get(pk=request.data['id'])
        except truck.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = truckdetailSerializer(truck_details, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    else:
...
@api_view(['GET', 'POST', 'PUT'])
def newtrip_detail(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            try:
                newtrip_details = NewTripDetails.objects.get(pk=pk)
                serializer = NewTripdetailSerializer(newtrip_details) 
                return Response(serializer.data)
            except newtrip_details.DoesNotExist:
                return Response(status=status.HTTP_404_NOT_FOUND)
        else:
            newtrip_details = NewTripDetails.objects.all()
            serializer = NewTripdetailSerializer(newtrip_details, many=True)
            return Response(serializer.data)
    elif request.method == 'POST':
        serializer = NewTripdetailSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    elif request.method == 'PUT':
        try:
            newtrip_details = NewTripDetails.objects.get(pk=request.data['id'])
        except NewTripDetails.DoesNotExist:  # Use the correct exception class
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = NewTripdetailSerializer(newtrip_details, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    else:
...
@api_view(['GET', 'POST', 'PUT'])
def data_detail(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            try:
                data_details = Data.objects.get(pk=pk)
                serializer = DataDetailSerializer(data_details) 
                return Response(serializer.data)
            except data_details.DoesNotExist:
                return Response(status=status.HTTP_404_NOT_FOUND)
        else:
            data_details = Data.objects.all()
            serializer = DataDetailSerializer(data_details, many=True)
            return Response(serializer.data)
    elif request.method == 'POST':
        serializer = DataDetailSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    elif request.method == 'PUT':
        try:
            data_details = Data.objects.get(pk=request.data['id'])
        except data_details.DoesNotExist:  # Use the correct exception class
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = DataDetailSerializer(data_details, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    else:
...
def owner_detail(request):
    if request.method == 'GET':
        # try:
        data_details = OwnerDetail.objects.all()
        serializer = OwnerDetailSerializer(data_details, many=True) 
        #         return Response(serializer.data)
        # except data_details.DoesNotExist:
        #         return Response(status=status.HTTP_404_NOT_FOUND)
    elif request.method == 'POST':
...
@api_view(['GET'])
def truck_owner(request,pk):
    if request.method == 'GET':
        filtered_detail = filter_data("truck","ownerid", pk)
        return (filtered_detail)

# @api_view(['GET'])
# def trip_driver(request,pk):
#     if request.method == 'GET':
#         # Get data from database using pk 
#         trip_detail = NewTripDetails.objects.filter(driver_id = pk).values()   #filter data according to owner id
#         if not trip_detail:
#             trip_detail = get_object_or_404(NewTripDetails, driver_id=pk).values()
#         else:
#             trip_detail = NewTripDetails.objects.filter(driver_id = pk).values()
#         return Response(trip_detail)
    
        
@api_view(['GET'])
def trip_driver(request,pk):
    if request.method == 'GET':
        filtered_detail = filter_data("NewTripDetails","driver", pk)
        return (filtered_detail)
    
@api_view(['GET'])
def driver_owner(request,pk):
    if request.method == 'GET':
        filtered_detail = filter_data("DriverDetail","ownerid", pk)
        return (filtered_detail)
    
@api_view(['GET'])
def NewTrip_owner(request,pk):
    if request.method == 'GET':
        filtered_detail = filter_data("NewTripDetails","ownerid", pk)
        return (filtered_detail)
    
@api_view(['GET'])
def NewTrip_Truck(request,pk):
    if request.method == 'GET':
        filtered_detail = filter_data("NewTripDetails","truckid", pk)
        return (filtered_detail)